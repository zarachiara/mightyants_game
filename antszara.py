...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        self.entrance = None  # A Place # always start with entrance as None
        # Phase 1: Add an entrance to the exit
        "*** YOUR CODE HERE ***"
        if self.exit: 
            self.exit.entrance = self # exit's entrance is set to self. 
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant():
            ...
            if self.ant is None: 
                self.ant = insect
            elif self.ant.can_contain(insect):# If the Ant currently occupying this Place can contain the Ant we are trying to add, then simply tell it to do so.
                self.ant.contain_ant(insect)
            elif insect.can_contain(self.ant): #If the Ant we are trying to add can contain the Ant currently occupying this Place, then have it do so and set this Place's ant to be the newly added Ant. ??
                insect.contain_ant(self.ant) # i dont know but this is kind of lazy
                self.ant = insect # have it do so
            else: 
...
class Insect(object):
    ...
    watersafe = False # new attribute A4 
...
class Ant(Insect):
    ...
    def can_contain(self, other): 
        if self.container and not other.container: 
            if self.ant is None: # this does not already return an ant
                return True 
        return False
...
class HarvesterAnt(Ant):
    ...
    def action(self, colony):
        ...
        """
        "*** YOUR CODE HERE ***"
        colony.food += 1 # food is an attribute of AntColony but how did it inherit this?  
...
class ThrowerAnt(Ant):
    ...
    food_cost = 4
    damage = 1
    min_range = 0 # from question B5 implement min and max ranges as base class
    max_range = 10 
    ...
    def nearest_bee(self, hive):
        ...
        """
        "*** YOUR CODE HERE ***"
        current_place = self.place # when it says the hive we are assuming it is in that place. Thrower Ant doesnt wait for bee to go to hive until killing it 
        i = 0 
        while current_place !=[] and current_place is not hive # when there is a bee and the thrower ant is not in the same place
            if counter >= self.min_range and counter <= self.max_range # when counter is implemented from B5
                if current_place.bees != []: 
                    return random_or_none(current_place.bees) #For each place, return a random bee if there is any
            current_place = current_place.entrance #or consider the next place that is stored as the current place's entrance.
            i +=1
...
class Water(Place): # Question A4 new type of place called water
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0: 
            bees_copy = list(self.place.bees)# iterate OVER A COPY! 
            for bee in bees_copy: # for loop the copy of bees in place
                bee.reduce_armor(3) # once amount reaches 0 # is there a better way to do this?
...
class WallAnt(Ant):
    ...
    def __init__(self):
        "*** YOUR CODE HERE ***"
        Ant.__init__(self, 4) # what is a better way to do this again? 
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        "*** YOUR CODE HERE ***"
        if self.place.bees: # same exact thing as FireAnt except armor is not being reduced each time
